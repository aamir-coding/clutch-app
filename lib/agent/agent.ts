...
class ClutchAgent {
  ...
  async buildContext(userId: string): Promise<{ currentDatetime: string; workHours: string; atRiskSummary: string }> {
    const user = await firestoreService.getUser(userId);
    const atRiskTasks = await firestoreService.getAtRiskTasks(userId, 48);
    
    const currentDatetime = new Date().toLocaleString('en-US', {
      weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit'
    });
    
    const workHours = user?.workHours ? `${user.workHours.start} to ${user.workHours.end}` : '09:00 to 18:00';
    
    const hoursUntil = (deadline: Date | string) => (new Date(deadline).getTime() - Date.now()) / (1000 * 60 * 60);
    
    const atRiskSummary = atRiskTasks.length === 0 
      ? 'None' 
      : atRiskTasks.map(t => `${t.title} (due ${hoursUntil(t.deadline).toFixed(1)}h)`).join(', ');
  ...
  async *runStream(userId: string, userMessage: string, history: AgentMessage[]): AsyncGenerator<AgentStreamEvent> {
    try {
        ...
        for await (const chunk of result.stream) {
          ...
          for (const part of parts) {
            if (part.functionCall) {
              ...
              const toolResult = await executeAgentTool(part.functionCall.name, part.functionCall.args as Record<string, unknown>, userId);
  ...
  async generateBattlePlan(userId: string, horizonDays: number): Promise<WorkPlan> {
    ...
    // In a real implementation, we'd fetch actual calendar events here too.
    // For now, providing a placeholder or empty calendar JSON
    const calendarJson = '[]'; 
    ...
    try {
      ...
      throw new Error('Battle plan parse failed');
    }
  }
}
