...
import { useCalendar, CalendarEvent, CalendarSession } from '@/lib/hooks/useCalendar';
import { useTasks } from '@/lib/hooks/useTasks';
import { Task } from '@/lib/types';
import { TaskDetailSheet } from '@/components/dashboard/Modals';
import {
  ChevronLeft, ChevronRight,
  Zap, Clock, Plus, Loader2, X, AlertCircle
} from 'lucide-react';
...
export default function TimelinePage() {
  ...
  useEffect(() => {
    if (setPageTitle) setPageTitle('Timeline');
  }, [setPageTitle]);
  ...
  const getTimelineItemStyle = (startStr: string, endStr: string) => {
    ...
    return { top: `${topOffset}px`, height: `${calculatedHeight}px` };
  };
  ...
  const getDayFreeSlots = (day: Date, dayEvents: CalendarEvent[], daySessions: CalendarSession[]) => {
    const busyPeriods: { start: number; end: number }[] = [];

    dayEvents.forEach((e) => {
      const s = new Date(e.start);
      const end = new Date(e.end);
      busyPeriods.push({ start: s.getHours() + s.getMinutes() / 60, end: end.getHours() + end.getMinutes() / 60 });
    });

    daySessions.forEach((s) => {
      const start = new Date(s.start);
      const end = new Date(s.end);
      busyPeriods.push({ start: start.getHours() + start.getMinutes() / 60, end: end.getHours() + end.getMinutes() / 60 });
    });
    ...
    return potentialSlots.filter((slot) =>
      !busyPeriods.some((busy) => slot.startHour < busy.end && slot.endHour > busy.start)
    );
  };
  ...
  const handleOpenScheduleModal = (day: Date, startHour: number, endHour: number) => {
    ...
    setSchedulingSlot({ date: targetDate, startHour, durationMinutes: (endHour - startHour) * 60 });
  ...
  const activeTasksToSchedule = useMemo(() => tasks.filter((t) => t.status !== 'completed'), [tasks]);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-900 pb-5">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center bg-[#0C0C14] border border-[#1E1E2E] p-1 rounded-xl">
            <button onClick={handlePrevWeek} className="p-2 hover:bg-white/5 text-slate-400 hover:text-white transition rounded-lg cursor-pointer">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <span className="px-3 text-xs font-bold font-mono text-slate-300 min-w-[120px] text-center">{headerLabel}</span>
            <button onClick={handleNextWeek} className="p-2 hover:bg-white/5 text-slate-400 hover:text-white transition rounded-lg cursor-pointer">
        ...
        <div className="min-w-[900px]">
          ...
          <div className="grid grid-cols-8 border-b border-slate-900">
            ...
            {weekDays.map((day) => {
              ...
              return (
                <div key={day.toISOString()} className="p-3.5 text-center flex flex-col items-center justify-center border-r border-slate-900/40 relative">
                  ...
                  {isToday && <div className="absolute bottom-0 left-4 right-4 h-0.5 bg-indigo-500 rounded-full" />}
                </div>
              );
            })}
          </div>
            ...
            <div className="border-r border-slate-900 bg-[#08080C] flex flex-col relative" style={{ height: `${HOURS.length * HOUR_HEIGHT}px` }}>
              {HOURS.map((hour, idx) => {
                ...
                return (
                  ...
                  >
                    <span className="text-[10px] font-mono text-slate-550 pt-1">{displayHour}:00 {suffix}</span>
                  </div>
                );
              })}
            </div>
            ...
            {weekDays.map((day) => {
              ...
              return (
                ...
                >
                  {/* Grid Lines */}
                  {HOURS.map((_, idx) => (
                    <div key={idx} className="absolute left-0 right-0 border-b border-slate-900/45 pointer-events-none" style={{ top: `${idx * HOUR_HEIGHT}px`, height: `${HOUR_HEIGHT}px` }} />
                  ))}
                  ...
                  {dayEvents.map((evt) => (
                    ...
                    >
                      <span className="text-[10px] font-semibold text-slate-200 truncate leading-tight">{evt.summary}</span>
                  ...
                  {daySessions.map((session) => (
                    ...
                    >
                      <div className="flex flex-col min-w-0">
                        ...
                        <span className="text-[10px] font-extrabold text-white truncate mt-1 leading-snug">{session.taskTitle}</span>
                      </div>
                  ...
                  {freeSlots.map((slot) => {
                    ...
                    const heightOffset = (slot.endHour - slot.startHour) * HOUR_HEIGHT;
                    return (
                      <button
                        key={`${slot.startHour}-${slot.endHour}`}
                        onClick={() => handleOpenScheduleModal(day, slot.startHour, slot.endHour)}
                        style={{ top: `${topOffset + 3}px`, height: `${heightOffset - 6}px` }}
                      ...
                      >
                        <span className="text-[9px] font-mono font-bold text-slate-500 group-hover:text-indigo-400 uppercase tracking-widest">+ BOOK CLUTCH FOCUS</span>
                        <span className="text-[8px] text-slate-600 group-hover:text-indigo-500 uppercase font-mono tracking-wider">{slot.startHour}:00 - {slot.endHour}:00</span>
                      </button>
                    );
                  })}
          ...
          <div className="relative w-full max-w-sm bg-[#0C0C14] border border-slate-800 rounded-2xl shadow-2xl p-6 text-slate-100 flex flex-col gap-5">
            <div className="flex items-center justify-between pb-3 border-b border-slate-900">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-bold uppercase tracking-wider font-mono text-white">Schedule CLUTCH block</h3>
              </div>
            ...
            <div className="space-y-4">
              ...
              {activeTasksToSchedule.length === 0 ? (
                <div className="p-4 rounded-xl border border-yellow-500/15 bg-yellow-500/5 text-center space-y-2">
                  ...
                  <p className="text-[10px] text-slate-400">Create a task first in the Command Center before scheduling focus blocks.</p>
                </div>
              ) : (
