...
import CrisisOverlayProvider from '@/components/layout/CrisisOverlayProvider';
import {
  ShieldAlert, Zap, LayoutGrid, Award,
...
export default function DashboardLayout({
  ...
  return (
    <AuthGuard>
      {/*
       * CrisisOverlayProvider must live inside AuthGuard (so the user is
       * authenticated before any crisis state fires) but outside the scroll
       * container (its overlay is position:fixed and needs to escape the
       * overflow:hidden on the inner layout divs).
       *
       * It reads `crisisTaskId` from uiStore and renders the full-screen
       * CrisisMode overlay whenever the agent emits a `crisis_activated` event.
       */}
      <CrisisOverlayProvider>
