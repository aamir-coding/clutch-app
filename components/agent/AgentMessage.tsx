    ...
    return (
      <div className="flex flex-row-reverse items-start gap-3">
        ...
        <div className="flex flex-col items-end">
          ...
          {/*
           * formatTime from lib/utils/formatDate avoids the
           * locale-sensitive hydration mismatch that toLocaleTimeString()
           * causes between SSR (Node.js) and the browser.
           */}
              ...
              components={{
                code(props: any) {
                  ...
                  return (
                    <code
                      {...rest}
                      className={`${className ?? ''} bg-[#0A0A0F] px-1 py-0.5 rounded text-indigo-300 font-mono text-xs`}
                    >
                      {children}
                    </code>
                  );
