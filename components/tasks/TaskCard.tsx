...
import { Clock, CheckCircle2, Circle, ArrowUpRight } from 'lucide-react';
...
export default function TaskCard({ task, onUpdate, onOpenDetails, onDelete }: TaskCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start gap-3">
        <div className="space-y-1 flex-1">
          <div className="flex flex-wrap items-center gap-2">
            <span
              className={`text-[10px] font-mono font-bold uppercase tracking-wider px-2 py-0.5 rounded border ${getPriorityBadge(priority)}`}
            >
              {priority}
            </span>
            {totalSubtasks > 0 && (
          ...
          <h3
            className={`text-sm font-bold leading-snug font-sans group-hover:text-indigo-300 transition-colors
      ...
      {/* Deadline — formatDate avoids locale-sensitive hydration mismatches */}
